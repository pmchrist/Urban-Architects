...
class PercolationModel2D(object):    
    ...
    migration_threshold = 0.35       # Without simple migration all of the environment events much more clear
    burnrate = 0.0
    growthrate = 0.0
    ...
    def update_fitness(self):
        ...
        # self.fitness[(i - 1) % self.size, j] =  np.random.rand()
        # self.fitness[(i + 1) % self.size, j] =  np.random.rand()
        # self.fitness[i, (j - 1) % self.size] =  np.random.rand()
        # self.fitness[i, (j + 1) % self.size] =  np.random.rand()
                ...
                if self.type[i,j] == 0 and self.fitness[i,j] < self.migration_threshold:
                    ...
                    for k in range(len(destinations)):
                        if self.pop_dens[destinations[k][0], destinations[k][1]] < 1.0 - self.emmigration_size and self.type[destinations[k][0], destinations[k][1]] == 0:
                            self.next_pop_dens[destinations[k][0], destinations[k][1]] += self.pop_dens[i,j]*self.emmigration_size
                            self.next_pop_dens[i,j] -= self.pop_dens[i,j]*self.emmigration_size
                            simple_migration += self.pop_dens[i,j]*self.emmigration_size
