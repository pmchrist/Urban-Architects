...
class PercolationModel_BA(object):    
    '''
    Object that calculates and displays behaviour of 2D cellular automata
    '''

    # Just keeping global variables for reference:
    migration_threshold = 0.3       # Without simple migration all of the environment events much more clear
    burnrate = 0.2
    growthrate = 0.8
    ...
    # We are combining available energy/density
    ...
    def land_migration(self):
        ...
        for i in range(self.N):
            for j in range(self.N):
                # If fitness is below a certain threshold, trigger a migration event
                if self.fitness[i, j] < self.migration_threshold:
                    #Bak-Sneppen part:
                    # Randomly assign new fitness values to the cell and its neighbors
                    neighbors = self.getMooreNeighbourhood(i, j)
                    for neighbor in neighbors:
                        self.fitness[neighbor[0], neighbor[1]] = np.random.rand()
                    self.fitness[i, j] = np.random.rand()
                    # Sort neighbors by fitness
                    neighbors.sort(key=lambda x: self.fitness[x[0], x[1]], reverse=True)
                    # Move a portion of the population from the current cell to the neighbor with the highest fitness
                    migration_size = self.emmigration_size * (self.fitness[neighbors[0][0], neighbors[0][1]] - self.fitness[i, j])
                    if self.pop_dens[i, j] > migration_size:
                        self.next_pop_dens[neighbors[0][0], neighbors[0][1]] += migration_size
                        self.next_pop_dens[i, j] -= migration_size
                        simple_migration += migration_size
