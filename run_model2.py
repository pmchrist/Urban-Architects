from BS_CA import PercolationModel_BA
import matplotlib.pyplot as plt
from time import sleep
from numpy import log10
from config import N, temp, nzeros
import powerlaw
import pandas as pd

# Create the percolation model, and seed four colony sites at the centre
cell= PercolationModel_BA(N, temp)
...
while istep < nsteps:    
    ...
    plt.draw()
    plt.savefig('./results2/pop/step_pop'+str(istep).zfill(nzeros)+".png")      
    ...
    #hist = ax.pcolor(cell.nextgrid,edgecolors='black', cmap='binary')
    plt.draw()
    plt.savefig('./results2/energy/step_energy'+str(istep).zfill(nzeros)+".png")     
    ...
    #hist = ax.pcolor(cell.nextgrid,edgecolors='black', cmap='binary')
    plt.draw()
    plt.savefig('./results2/water/type'+str(istep).zfill(nzeros)+".png")     
